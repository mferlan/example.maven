...
SET DOWNLOAD_FOLDER=\\git\platform\downloads\tycho.sample.osgi.additional
IF not exist %DOWNLOAD_FOLDER% (
REM we are not on the eclipsesource build machine. for testing, let's
REM deploy the build inside the builds folder of git\platform\downloads\tycho.sample.osgi.additional
   SET DOWNLOAD_FOLDER=%ROOT_FOLDER%\target\downloads\tycho.sample.osgi.additional
